...
export function AnalyticsClient({ 
	...
	const fetchSales = useCallback(async (startDate: string, endDate: string) => {
		const url = `/api/analytics?start=${encodeURIComponent(startDate)}&end=${encodeURIComponent(endDate)}`;
		const res = await fetch(url, { cache: 'no-store' });
		if (!res.ok) {
			throw new Error('Failed to fetch sales');
		}
		return (await res.json()) as DailySalesData[];
	}, []);
	...
	const handleViewModeChange = useCallback(async (mode: 'daily' | 'weekly' | 'monthly') => {
		...
		try {
			...
			const newData = await fetchSales(startDate, endDate);
		...
		<div className='space-y-4 lg:space-y-6 pb-4 lg:pb-6'>
			...
			<div className='grid gap-3 lg:gap-4 grid-cols-2 lg:grid-cols-4'>
				{metricsCards.map((card) => (
					<div
						key={card.title}
						className='bg-card rounded-lg lg:rounded-xl border p-3 sm:p-4 lg:p-6 hover:shadow-md transition-all duration-200'>
						<div className='flex items-center justify-between mb-2 sm:mb-3 lg:mb-4'>
							<div className={`p-1.5 lg:p-2 rounded-md lg:rounded-lg ${card.bgColor}`}>
								<div className={card.color}>{card.icon}</div>
							</div>
							{card.change !== 0 && (
								<Badge variant={card.change > 0 ? 'default' : 'destructive'} className='gap-0.5 lg:gap-1 text-xs px-1.5 lg:px-2'>
									{card.change > 0 ? (
						...
						<div>
							<p className='text-xs lg:text-sm font-medium text-muted-foreground mb-1 line-clamp-2'>
								{card.title}
							</p>
							<p className='text-base sm:text-lg lg:text-2xl font-bold mb-1 lg:mb-2 truncate'>
								{card.value}
							</p>
			...
			<div className='bg-card rounded-xl border'>
				<AnalyticsChart initialData={salesData} onViewModeChange={handleViewModeChange} loading={loading} />
			</div>
					...
					{initialMetrics.topSellingItems && initialMetrics.topSellingItems.length > 0 ? (
						<div className='space-y-2 lg:space-y-3 max-h-80 overflow-y-auto'>
							{initialMetrics.topSellingItems.slice(0, 5).map((item, index) => (
								<div key={item.id} className='flex items-center justify-between p-2 lg:p-3 bg-muted rounded-md lg:rounded-lg'>
									...
									<div className='text-right flex-shrink-0'>
										<p className='font-semibold text-sm lg:text-base'>₹{item.revenue.toFixed(2)}</p>
									</div>
								</div>
							))}
						</div>
					...
					<div className='space-y-3 lg:space-y-4'>
						<div className='flex items-center justify-between p-2 lg:p-3 bg-green-50 rounded-md lg:rounded-lg'>
							<div className='flex items-center gap-2 lg:gap-3 min-w-0 flex-1'>
								...
								<span className='text-xs lg:text-sm font-medium truncate'>Orders Today</span>
							</div>
						...
						<div className='flex items-center justify-between p-2 lg:p-3 bg-emerald-50 rounded-md lg:rounded-lg'>
							<div className='flex items-center gap-2 lg:gap-3 min-w-0 flex-1'>
								...
								<span className='text-xs lg:text-sm font-medium truncate'>Revenue Today</span>
							</div>
							<span className='font-semibold text-sm lg:text-base flex-shrink-0'>
						...
						<div className='flex items-center justify-between p-2 lg:p-3 bg-emerald-50 rounded-md lg:rounded-lg'>
							<div className='flex items-center gap-2 lg:gap-3 min-w-0 flex-1'>
								<div className='w-2 h-2 bg-emerald-500 rounded-full flex-shrink-0'></div>
								<span className='text-xs lg:text-sm font-medium truncate'>Avg Order Value</span>
							</div>
							<span className='font-semibold text-sm lg:text-base flex-shrink-0'>
